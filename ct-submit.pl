...
pod2usage(-exitval => 0, -verbose => 2) unless (@pem);
...
if (defined($extbuf)) {
    ...
    foreach my $sct (@sct) { $size += 2 + length($sct); }
    
    open(my $fh, ">$extbuf") or die("Cannot open $extbuf for writing");
    binmode($fh);
    
    foreach my $sct (@sct) { print $fh pack('(S a' . length($sct) . ')>', length($sct), $sct); }
...
Output folder - all responses from log querie will be save in individual .sct files inside this folder
